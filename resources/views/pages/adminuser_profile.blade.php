...
<main class="w-[86.6%] absolute top-40 left-64 p-20 flex flex-col gap-y-8 z-0">
    <div class="flex justify-between mx-40">
        ...
        @if ($student->status === 1)
            <a class="w-20 h-10 bg-[#168753] rounded-md text-white hover:bg-green-900 flex items-center justify-center" href="{{ route('print.pdf', ['id' => $student->id]) }}">
                Export
            </a>
        @endif
    </div>
    <div class="shadow-md rounded-lg bg-white mx-80 px-20 py-10">
        ...
        @if ($student->status===0)
            <div class="flex flex-row w-full justify-center mt-10 gap-x-10">
                ...
                <form action="{{route('decline.applicant')}}" method="post" class="hidden">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="student_id" value='{{$student->id}}'>
